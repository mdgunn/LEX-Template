...
void main(void)
{
	...
	float blendammount = 1-ex_color.a;
	vec3 cubecoord = reflect(normalize(ex_vertexposition.xyz + ex_VertexCameraPosition), normalize(cameranormalmatrix*ex_normal).xyz);
	outcolor=mix(outcolor, texture(texture4,cubecoord),blendammount);
